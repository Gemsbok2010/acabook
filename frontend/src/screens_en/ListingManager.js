...
const ListingManager = () => {
  ...
  const pagePrevious = async () => {
    const res = await fetch(
      process.env.REACT_APP_BACKEND_URL +
        `api/intllistings/listingmanager?page=${page <= 0 ? 0 : page - 1}` +
        "&sortBy=" +
        sort +
        "&contract=" +
        contract +
  ...
  const pageNext = async () => {
    const res = await fetch(
      process.env.REACT_APP_BACKEND_URL +
        `api/intllistings/listingmanager?page=${
  ...
  const IntermediateButtons = async (id) => {
    const res = await fetch(
      process.env.REACT_APP_BACKEND_URL +
        `api/intllistings/listingmanager?page=${id + 1}` +
        "&contract=" +
        contract +
        "&subjects=" +
        subjects +
  ...
  const sorting = async (ascDesc) => {
    ...
    if (ascDesc === false) {
      const res = await fetch(
        process.env.REACT_APP_BACKEND_URL +
          "api/intllistings/listingmanager?sortBy=asc" +
          "&contract=" +
          contract +
          "&subjects=" +
          subjects +
    ...
    if (ascDesc === true) {
      ...
      const res = await fetch(
        process.env.REACT_APP_BACKEND_URL +
          "api/intllistings/listingmanager?sortBy=desc" +
          "&contract=" +
          contract +
          "&subjects=" +
          subjects +
  ...
  const highlight = async (slug) => {
    ...
    const res = await fetch(
      process.env.REACT_APP_BACKEND_URL +
        `api/intllistings/candidates?sortBy=` +
        sort +
        "&contract=" +
        contract +
        "&subjects=" +
  ...
  const activeAd = async (e, slug) => {
    ...
    const res = await fetch(
      process.env.REACT_APP_BACKEND_URL +
        `api/intllistings/sleepAd/${slug}/?sortBy=` +
        sort +
        "&page=" +
        page,
      {
        ...
        body: JSON.stringify({ isTeacherJob: false, isDeletedJob: false }),
      }
    );
    const data = await res.json();
  ...
  const onReject = async (e, slugId, nanoId) => {
    ...
    const res = await fetch(
      process.env.REACT_APP_BACKEND_URL +
        `api/intllistings/reject/${slugId}/${nanoId}?sortBy=` +
        sort +
        "&page=" +
        page,
      {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      ...
      const res = await fetch(
        process.env.REACT_APP_BACKEND_URL +
          "api/intllistings/listingmanager?" +
          "contract=" +
          contract +
          "&subjects=" +
          subjects +
        ...
        <section>
          <div className="fix-Container">
            <div className="row">
              ...
              {noOfCases.length === 0 ? (
                <div className="results">You have posted 0 listing</div>
              ) : noOfCases > 1 ? (
                <div className="results">
                  You have posted {noOfCases} listings
                </div>
              ) : (
                <div className="results">
                  You have posted {noOfCases} listing
                </div>
              )}
            </div>
          </div>
                  ...
                  <div className="row">
                    <div className="states_flex">
                      ...
                      <input
                        ...
                        value="澎湖縣"
                      />
                      <label htmlFor="q">Penghu County</label>
                    </div>
                  </div>
                </div>
                <input
                    ...
                    return (
                      <div className="ads" key={listing._id}>
                        <div className="leftmessage">
                          ...
                          <h3>Posted: {listing.todaysDate}</h3>
                          <p>{listing.about}</p>
                        </div>
                      </div>
                    );
              ...
              {ReactSession.get("slug") ? (
                ...
                >
                        ...
                        <div className="rightmessage">
                          <div className="bookdateBar">
                            <ExternalLink
                              ...
                              href={
                                process.env.REACT_APP_BACKEND_URL +
                                `api/intlteachers/resumeCandidate/${candidate.nanoId}/${candidate.slugId}`
                              }
                            >
                              <div className="previewme">Resume</div>
                            </ExternalLink>
                          ...
                          <div className={"parentfirstSecond"}>
                            <div className="firstBox">
                              <p className="home">I live: {candidate.state}</p>

                              <p className="pig">
                                Hourly rate: NTD{candidate.yourpay}
                              </p>
                            </div>
                            <div className="secondBox">
                              {candidate.hometutor === "我不太方便" ? (
                                <p className="forbid">Home Tutoring?</p>
                              ) : (
                                <p className="yes">Home Tutoring?</p>
                              )}
                              {candidate.onlinetutor === "我不太方便" ? (
                                <p className="forbid">Online Tutoring?</p>
                              ) : (
                                <p className="yes">Online Tutoring?</p>
                              )}
                            </div>
                          </div>
                        </div>
                  ...
                  {candidates.length === 0 && (
                    ...
                    >
                      Sorry, no applicants at the moment.
                    </div>
                  )}
                </div>
              ) : (
