...
const Calendar = () => {
  ...
  if (nav !== 0) {
    const what = dt.setMonth(new Date().getMonth() + nav);
    console.log(what);
  }
  ...
  const weekGone = Math.floor((day - paddingDays) / 7);
  const daysGone = weekGone * 7;
  const lastDayOfWeek = Math.ceil((day - paddingDays) / 7) * 7;
  ...
  return (
    <>
      <HelmetProvider>
        <Helmet>
          <title>My Calendar | Acabook</title>
              ...
              {squares.map((square) => {
                return (
                  <div
                    ...
                    className={
                      parseInt(square.key) < daysGone + 1 && month === maand
                        ? "weekGone day"
                        : parseInt(square.key) < day + paddingDays &&
                          month === maand
                        ? "inactive day"
                        : parseInt(square.key) >= lastDayOfWeek + 1 &&
                          month === maand
                        ? "weekGone day"
                        : "day"
                    }
