...
const Step1 = () => {
  ...
  const onSubmit = (e) => {
    e.preventDefault();
    fetch(process.env.REACT_APP_BACKEND_URL + "api/teachers/step1", {
      method: "POST",
      credentials: "include",
            ...
            <ul className="stepNav threeWide">
              <li>
                <Link className="active" style={{ fontWeight: "bold" }} to="#">
                  ...
                  <span className="active">家教基本資料</span>
                </Link>
              </li>
              <li>
                <Link
                  style={{
                    fontWeight: "bold",
                    cursor:
                      ReactSession.get("teacher_street") &&
          ...
          <form id="formOne" onSubmit={onSubmit}>
            ...
            <div className="personContent">
              <section className="buttonCard">
                {street && nationality && phone ? (
                  <input type="submit" className="btn-vori" value="下一步" />
                ) : (
                  <input
                    type="button"
                    disabled
