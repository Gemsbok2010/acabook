...
const {
  question6International,
  question7International,
...
router.get("/question3", async (req, res) => {
  Subject.paginate({}, {}).then(async (result) => {
    ...
    let contract;
    if (req.query.contract === "Languages") {
      contract = "語言";
    } else if (req.query.contract === "Other") {
      contract = "一般課程";
    } else if (req.query.contract === "University") {
      contract = "大學";
    } else if (req.query.contract === "School") {
      contract = "學校";
    } else {
      contract = "樂器";
    }
...
router.put("/question6", async (req, res, next) => {
  const { error } = question6International(req.body);
...
router.put("/question7", async (req, res, next) => {
  const { error } = question7International(req.body);
  if (error) return res.status(400).json({ invalid: error.details[0].message });
  res.send(req.body);
});
...
router.post("/question10", async (req, res, next) => {
  ...
  let contractType;
  if (req.body.contractType === "Languages") {
    contractType = "語言";
  } else if (req.body.contractType === "Other") {
    contractType = "一般課程";
  } else if (req.body.contractType === "University") {
    contractType = "大學";
  } else if (req.body.contractType === "School") {
    contractType = "學校";
  } else {
    contractType = "樂器";
  }
  ...
  try {
    ...
    res.redirect(process.env.FRONTEND_URL + "dashboard/en");
  }
});
...
router.get("/candidates", async (req, res) => {
  Listing.paginate({}, {}).then(async (result) => {
    ...
    if (
      req.query.contract === "大學" ||
      req.query.contract === "學校" ||
      req.query.contract === "樂器" ||
      req.query.contract === "語言" ||
      req.query.contract === "其他"
    ) {
      let set = {};
      set["seen"] = true;
...
router.get("/adPosts/:slug", async (req, res) => {
  ...
  if (post === null) {
    ...
    res.redirect(process.env.FRONTEND_URL + "Ad_details_en/" + req.params.slug);
  }
});
...
router.get("/edit/:slug", async (req, res) => {
  ...
  if (post === null) {
    ...
    res.redirect(
      process.env.FRONTEND_URL + "ListingEdit_en/" + req.params.slug
    );
  }
});
