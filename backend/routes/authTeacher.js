...
router.get("/resume/:teacherId", async (req, res) => {
  ...
  if (teacher === null) {
    ...
    res.redirect(process.env.FRONTEND_URL + "resume/" + req.params.teacherId);
  }
});
...
// update teacher CV (from teacher_cv.ejs)
...
// update teacher Profile (from teacher_profile.js)
...
router.post("/upload", async (req, res) => {
  upload(req, res, async (err) => {
    ...
    if (req.file === undefined) {
      res.json({
        invalid: "無法執行: 上傳圖片檔案超過容量限制或是檔案類別不被允許上傳.",
      });
    } else {
